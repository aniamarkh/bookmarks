...
import type { Category, DataNode } from "../types";
...
const checkIfHidden = (node: DataNode) => {  
  return store.hidden.indexOf(node) < 0 ? true : false;
}
...
<template>
  <div class="subcategory-wrapper" v-if="!showEditCategoryForm">
    ...
    <div class="subcategory-btns" v-if="settings.edit">
      ...
      <button v-if="checkIfHidden(subcategory as DataNode)" class="category-btn" @click="hideCategory(subcategory as DataNode)">
