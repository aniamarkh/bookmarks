...
<template>
  <div class="subcategory-wrapper" v-if="!showEditCategoryForm">
    <h4 class="subcategory-title" @click="toggleSubcat(subcategory.id)">
      <div v-if="!isClosed(subcategory.id)" class="icon">
        <svg
          ...
          viewBox="0 2 40 40"
        >
          <path d="M17.167 27.167V12.833L24.333 20Z" />
        </svg>
      </div>
      <div v-if="isClosed(subcategory.id)" class="icon">
        <svg
          ...
          viewBox="0 2 40 40"
        >
          <path d="m20 24.333-7.167-7.166h14.334Z" />
        </svg>
      </div>
      {{ subcategory.title }}
    </h4>
    <div class="subcategory-btns" v-if="settings.edit">
      <button
        class="category-btn"
        @click="showAddBookmarkForm = !showAddBookmarkForm"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 5 40 40"
      ...
      <button
        class="category-btn"
        @click="showEditCategoryForm = !showEditCategoryForm"
      >
      ...
      <button
        class="category-btn"
        v-if="isHidden(subcategory as Category)"
        @click="hideCategory(subcategory as Category)"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          height="20"
  ...
  <BookmarkForm
    ...
    @close-form="closeAddBookmarkForm"
  />
  <CategoryEditForm
